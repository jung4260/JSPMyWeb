...
public class UserController extends HttpServlet {
	...
	protected void doAction (HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException{
		...
		if(path.equals("/user/join.user")) {
			 ...
			 * 2.service에서는 먼저 login메서드를 이용해서 회원의 pw를 조회해서 가지고 나옵니다.
			 * 3.회원이 있다라는 것은 비밀번호가 일치한다는 의미
			 * 4.delete메서드를 호출시켜서 회원정보를 삭제하고, 세션 삭제하고, 홈화면으로 리다이렉트 
			 * 5.비밀번호가 일치하지 않은 경우 delete.jsp화면으로 메시지를 보내주세요.
			 */
			...
			if(result == 1) {
				...
				PrintWriter out = response.getWriter();
				out.println("<script>");
				out.println("alert('정상적으로 회원탈퇴 ㅠㅠ')");
